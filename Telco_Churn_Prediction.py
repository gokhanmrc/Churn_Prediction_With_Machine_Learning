...
# hayali bir telekom şirketi hakkında bilgi içerir. Hangi müşterilerin hizmetlerinden ayrıldığını, kaldığını veya hizmete
# kaydolduğunu içermektedir.
...
df = pd.read_csv("CaseStudy2-(TelcoChurn)/Telco-Customer-Churn.csv")
df.head()
df.shape
df.info()
...
df.select_dtypes(include='number').corrwith(df["Churn"]).sort_values(ascending=False)
...
df["TotalCharges"].fillna(df["TotalCharges"].median(), inplace=True)  # aylık ödenecek miktarlarıyla totalcharge doldurulailir veya 11 değişken drop edilebilir
